...
int computeUrgencyLinkid(int previd, int i){
...
//查找紧急程度的最小值
BreakLoopPoint findLowestUrgencyFlows(){
	//对于每一条业务流
				...
				if(num[k] == flowset.flows[i].route[j].linkid){
					for(int p = 1;p <= pregraph.vertex[k].flownumber; p++){
						if(pregraph.vertex[k].flowurgency[p].fid == i 
							...
							//尚未被调度，可行的调度，正常业务流								
							urgency[i] = computeUrgencyLinkid(k,p);
							Linkid[i] = k;
							flag = 1;
							break;								
	...
	int Maxindex = maxindex(urgency, flowset.fnumber);
	BreakLoopPoint blp;
	blp.fid = Maxindex;
...
//破环
void breakLoop(PreGraph& pregraph , BreakLoopPoint& blp,queue <unsigned int> &fifo){
...
//调度破环的业务流
void StackschedulingByUrgency (int previd,int fid,int index){
