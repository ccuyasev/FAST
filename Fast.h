...
#include <time.h>
//判断是否调度完成
bool areAllPreGVertexScheduled(PreGraph& pregraph){
	...
	cout << "开始调度破环的业务流" << endl;
	return true;
}
//计算节点previd上所有业务流的紧急程度
void computeUrgency( int previd, Network& network, FlowSet& flowset, PreGraph& pregraph){

	   for(int i = 1; i <= pregraph.vertex[previd].flownumber; i++){
           //已经调度的或被选做破环的业务流，无需计算紧急程度
		   if(!pregraph.vertex[previd].flowurgency[i].isfeasible ||
			   pregraph.vertex[previd].flowurgency[i].isscheduled ||
			   pregraph.vertex[previd].flowurgency[i].isbreakloop) continue;
		   ...
		   if(accdelay == MAX){
              ...
              if(urgency < 0){//说明该业务流无法调度
			      ...
			      pregraph.vertex[previd].flowurgency[i].offset = MAX;
				  ofstream dataFile;
				  dataFile.open("output\\urgency.txt", ofstream::app);
				  // 朝TXT文档中写入数据
...
void sortByUrgency(int previd){
	int flownumber = pregraph.vertex[previd].flownumber;//业务流个数
	for (int i = 1; i <= flownumber; i++) { //控制n-1趟冒泡
		//无法调度的或被选做破环的业务流，无需计算紧急程度
		if( !pregraph.vertex[previd].flowurgency[i].isfeasible || pregraph.vertex[previd].flowurgency[i].isbreakloop) continue; 
		for (int j = 1; j <= flownumber - i; j++){
			if(!pregraph.vertex[previd].flowurgency[j].isfeasible||pregraph.vertex[previd].flowurgency[j].isbreakloop) continue; 
...
//判断两条业务流是否发生冲突
bool isconflict (int i, int j, int previd){
...
//按照紧急程度确定发送时间
void schedulingByUrgency (int previd){

	int flownumber = pregraph.vertex[previd].flownumber;//业务流个数
	


	for (int i = 1; i <= flownumber; i++) {//计算可调度的业务流在previd标识的网络链路上的发送时间点
		if(!pregraph.vertex[previd].flowurgency[i].isfeasible || pregraph.vertex[previd].flowurgency[i].isbreakloop) continue;
	...
	if(flownumber == 1){
		int fidi = pregraph.vertex[previd].flowurgency[1].fid;
		while(pregraph.vertex[previd].flowurgency[1].offset < flowset.flows[fidi].offsetneed
		&& pregraph.vertex[previd].flowurgency[1].offset < flowset.flows[fidi].etedelay
		){
			pregraph.vertex[previd].flowurgency[1].offset++;
		}
	}
	//判断紧急程度比f_i高的业务流即下标小于i的业务流是否与f_i冲突
	for (int i = 2; i <= flownumber; i++) {
		if(!pregraph.vertex[previd].flowurgency[i].isfeasible || pregraph.vertex[previd].flowurgency[i].isbreakloop)continue; 
...
//						cout<<pregraph.vertex[previd].flowurgency[i].offset<<endl;
					...
					if(pregraph.vertex[previd].flowurgency[i].offset >= flowset.flows[fidi].etedelay){
						...
						// 朝TXT文档中写入数据
						dataFile << "fid:" << pregraph.vertex[previd].flowurgency[i].fid << endl;
						dataFile << "schedulingByUrgency:" << endl;
					...
					else if(number > periodi || number > 1400){
						//更新业务流在链路中的调度信息
						pregraph.vertex[previd].flowurgency[i].offset = MAX;
						pregraph.vertex[previd].flowurgency[i].isscheduled = true;//true:已经被调度；false:尚未被调度
						pregraph.vertex[previd].flowurgency[i].isfeasible = false;//true:可行的调度；false:无法被调度
			...
			int num = 0;
			for(int k = 1; k < i; k++){
				if(!pregraph.vertex[previd].flowurgency[k].isfeasible || pregraph.vertex[previd].flowurgency[k].isbreakloop) continue; 
				if(isconflict(i, k,previd)){
					num++;
					break;
				}	
			}
...
//将入度为0的节点入队列
void updatePreGVertexDegree(int previd,queue <unsigned int> &fifo, stack<BreakLoopPoint>& Stack){
	...
	//更新下一跳的累积传输时延	
	//找出节点的每条可调度业务流的下一跳节点
	for(int i = 1; i <= pregraph.vertex[previd].flownumber; i++){//对于每一条业务流
